...
func GetItemCF() *ItemCf {
	cf := ItemCf{
		DataPath:       "/Users/xiao_xiaoxiao/Desktop/web/mooc-web/core/data.csv",
		SaveTestPath:   "/Users/xiao_xiaoxiao/Desktop/web/mooc-web/core/testset.data",
		SaveTrainPath:  "/Users/xiao_xiaoxiao/Desktop/web/mooc-web/core/trainset.data",
		SaveSMXPath:    "/Users/xiao_xiaoxiao/Desktop/web/mooc-web/core/itemSimliarMatrix.data",
		TrainSetPecent: 0.75,
...
// 产生推荐电影的函数
func (cf *ItemCf) Recommend(uid string) []string {
	logrus.Error(" cf.TrainSetRec", cf.TrainSetRec)
	watchedMovices := cf.TrainSetRec[uid] //用户已经看过的电影
	logrus.Error(" cf.TrainSetRec", cf.TrainSetRec[uid])

	logrus.Error("cf.SimilierMatrix", cf.SimilierMatrix)
	logrus.Error("cf.SimMovieNum", cf.SimMovieNum)

	RecMap := map[string]float64{}
	for moviceId, _ := range watchedMovices {
		logrus.Error("cf.SimilierMatrix[moviceId]", cf.SimilierMatrix[moviceId])

		relatedMovies := cf.sortMap(cf.SimilierMatrix[moviceId], cf.SimMovieNum)
		logrus.Error("relatedMovies", relatedMovies)
	...
	mmap := cf.sortMap(RecMap, cf.RecMovieNum)
	res := []string{}
	for _, i := range mmap {
